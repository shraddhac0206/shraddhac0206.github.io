...
function App() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const sections = ['home', 'about', 'experience', 'projects', 'skills', 'honors', 'certifications', 'contact'];
  ...
  // Data Analytics Tooltip Component (descriptions removed)
  const AnalyticsTooltip = ({ children }) => (
  ...
  const projects = [
    ...
    },
    {
      ...
      description: "Unified data ingestion from raw files, APIs, and web-scraped feeds by building an automated ETL pipeline, contributing to a 25% faster deployment rate for downstream analytics initiatives. Integrated a CI/CD pipeline with automated testing using Docker containers and GitHub, reducing deployment time by 25%. Deployed PostgreSQL warehouse with Redis caching and Airflow orchestration, enabling 99.9% uptime and near real-time insights for downstream analytics.",
      technologies: [
        "Python",
        "Apache Kafka",
        "PostgreSQL",
    ...
    },
    {
      ...
      description: "Defined functional requirements for a Generative AI chatbot to replace manual research, resulting in a documented solution that reduced analyst research time by 60%. Performed a cost-benefit analysis projecting significant savings by automating frequent analyst queries, enabling reallocation of 2 full-time equivalent (FTE) hours daily toward strategic projects. Utilized Python/Streamlit to prototype the solution, demonstrating the ability to bridge technical development with a high-impact business need.",
    ...
    },
    {
      ...
      description: "Cleaned and structured a real-world layoffs dataset, applying SQL window functions (ROW_NUMBER + PARTITION BY) to eliminate 1,200+ duplicate records. Orchestrated the build-out of staging tables using PostgreSQL, guaranteeing raw data integrity for 100+ subsequent analyses and facilitating accurate, reproducible downstream insights for data science team. Prepared dataset for KPI-driven dashboards on layoff trends by company, industry, and time; enabling insights into cost-cutting patterns during downturns.",
    ...
    },
    {
      ...
      description: "Defined classification models (Random Forest, Extra Trees) that predicted telecom customer churn with 82% accuracy and 0.79 F1-score. Enhanced data preprocessing through application of One-Hot Encoding and stratified sampling, mitigating bias and bolstering predictive accuracy for traditionally underrepresented customer segments. Identified top churn drivers (Age, Balance, Activity level), helping business stakeholders simulate retention actions that could reduce churn risk by ~15%.",
    ...
    },
    {
      ...
      description: "Problem: Mined 1,451 employee records, pinpointing key factors behind departmental attrition rates, leading to actionable insights that the VP of HR could leverage to improve workforce stability. Approach: Analyzed records using Power BI, creating KPIs and DAX measures to uncover patterns by income, work-life balance, and promotion history. Insights: Championed the creation of two new HR policies focused on employee work-life balance, directly addressing the excessive overtime issue that was identified through detailed data analysis. Impact: Organised and presented three targeted retention strategies, focused on critical roles and demographics, to HR leadership and stakeholders at monthly meetings, resulting in immediate feedback and implementation.",
    ...
    },
    {
      ...
      description: "Problem: Manual sales order approval process in SAP S/4HANA led to processing delays and reduced customer satisfaction. Actions: Conducted stakeholder analysis, user story mapping, and GAP assessment; proposed SAP BRF+ workflow automation and e-commerce platform integration to streamline approvals. Results: Documented a complete functional specification aligning with SAP capabilities and business goals, targeting a 40% reduction in order processing time.",
    ...
    },
    {
      ...
      description: "Problem: Constructed comprehensive A/B test analyses to determine if a redesigned landing page increased customer conversion; findings led to website changes that fixed 3 of the biggest causes of bounces. Approach: Examined conversion rate disparities employing two-proportion Z-tests across time segments, revealing underperformance during weekend traffic and informing targeted website improvements. Insights: Discovered that the new landing page showed a 6.8% lift in conversion during weekdays, while weekend performance remained flat. Impact: Recommended reallocating ad spends toward weekday campaigns, projected to increase overall conversions by 12% without additional budget.",
  ...
  const honorsAchievements = [
    {
      title: "Selected Representative for UTA Business Analytics Program- July 2025",
      description: "Spearheaded communication strategies for the College of Business's global marketing initiative, recognized among the top 10 presenters for clarity and impact among a competitive pool of candidates."
    },
    {
      title: "AI-Powered Aviation Chatbot (Second Prize – UTA Business Symposium- March 2025)",
      description: "Lead a team which developed a Generative AI chatbot (OpenAI GPT + Streamlit) allowing users to query flight delays and route insights in natural language, reducing manual analysis time by 60% via an innovative solution design and won 2nd prize among 80+ teams."
    },
    {
      title: "Lifetime Member, Beta Gamma Sigma (BGS)",
      description: "The University of Texas at Arlington Chapter; invited and inducted into the international honor society for outstanding scholastic achievement in business, recognizing excellence among the top students in AACSB-accredited programs (2025–2026)."
    },
    {
  ...
  const certifications = [
    ...
    {
      name: "Certified Tableau Desktop Specialist",
      skills: "visual analytics, data connection, dashboard creation, and advanced chart types"
    },
    {
        ...
        <div className="nav-container">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="nav-links">
              {['home', 'about', 'experience', 'projects', 'skills', 'honors', 'certifications', 'contact'].map((section) => (
                <button
                  key={section}
                  ...
                  <div className="timeline-item">
                    ...
                    <div className="timeline-content">
                      <div className="timeline-header">
                        <h4 className="font-bold text-lg text-gray-700">Master of Science - Business Analytics</h4>
                      ...
                      <p className="text-sm text-gray-500 mb-3">GPA: 3.9/4.0 | Dec 2025</p>
                  ...
                  <div className="honor-badges">
                    {index === 0 ? (
                      <>
                        ...
                        <span className="honor-badge communication">Communication</span>
                      </>
                    ) : index === 1 ? (
                      <>
                      ...
                      <>
                        <span className="honor-badge academic">Academic</span>
                        <span className="honor-badge excellence">Excellence</span>
                      </>
                    ) : index === 3 ? (
                      <>
                        <span className="honor-badge award">Award</span>
                ...
                <div className="honor-metrics">
                  {index === 0 ? (
                    <div className="metric-item">
                      <span>Top 10 presenters</span>
                    </div>
                  ) : index === 1 ? (
                    <div className="metric-item">
                      <span>2nd prize among 80+ teams</span>
                    </div>
                  ) : index === 2 ? (
                    <div className="metric-item">
                      <span>AACSB-accredited programs</span>
                    </div>
                  ) : index === 3 ? (
                    <div className="metric-item">
                ...
                <p className="honor-description">
                  {honor.description.split('.').map((sentence, sentenceIndex) => {
                    ...
                    let enhancedSentence = sentence
                      .replace(/(communication strategies)/gi, '<strong>$1</strong>')
                      .replace(/(top 10 presenters)/gi, '<strong>$1</strong>')
                      .replace(/(clarity and impact)/gi, '<strong>$1</strong>')
                      .replace(/(Generative AI chatbot)/gi, '<strong>$1</strong>')
                      .replace(/(OpenAI GPT \+ Streamlit)/gi, '<strong>$1</strong>')
                      .replace(/(natural language)/gi, '<strong>$1</strong>')
                      .replace(/(60%)/gi, '<strong>$1</strong>')
                      .replace(/(innovative solution design)/gi, '<strong>$1</strong>')
                      .replace(/(2nd prize)/gi, '<strong>$1</strong>')
                      .replace(/(international honor society)/gi, '<strong>$1</strong>')
                      .replace(/(outstanding scholastic achievement)/gi, '<strong>$1</strong>')
                      .replace(/(AACSB-accredited programs)/gi, '<strong>$1</strong>')
